...
class TestStemmingResponder(unittest.TestCase):
    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_ff1.yml', record_mode='new_episodes')
    def test_follows_format1(self):
        comment = reddit.client().comment(id="dzpm1ak")
        response = StemmingResponder().respond(comment)
        self.assertIsNone(response)

    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_ff2.yml', record_mode='new_episodes')
    def test_follows_format2(self):
        comment = reddit.client().comment(id="dzrpx9y")
        response = StemmingResponder().respond(comment)
        self.assertIsNone(response)

    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_ff3.yml', record_mode='new_episodes')
    def test_follows_format3(self):
        comment = reddit.client().comment(id="e041767")
        response = StemmingResponder().respond(comment)
        self.assertIsNone(response)

    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_nvaa.yml', record_mode='new_episodes')
    def test_no_vote_at_all(self):
        comment = reddit.client().comment(id="dzrpoad")
        response = StemmingResponder().respond(comment)
        self.assertIsNone(response)

    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_nvo1.yml', record_mode='new_episodes')
    ...
    @vcr.use_cassette('tests/vcr/responders/stemming_responder.test_nvo2.yml', record_mode='new_episodes')
