...
class TestKamerledenHelper(unittest.TestCase):
    @vcr.use_cassette('tests/vcr/helpers/kamerleden_helper.test_eerste_kamer.yml', record_mode='new_episodes')
    def test_eerste_kamerleden(self):
        kamerleden = KamerledenHelper.eerste_kamerleden()
        self.assertEqual(len(kamerleden), 8)
 
    @vcr.use_cassette('tests/vcr/helpers/kamerleden_helper.test_tweede_kamer.yml', record_mode='new_episodes')
